...
var AppComponent = React.createClass ({
	...
	rearrange:function(centerIndex){
		var imgsArrangeArr=this.state.imgsArrangeArr,
			...
			imgsArrangeTopArr=[],
			topImgNum=Math.ceil(Math.random() * 2),//上端图片个数，取一个或者不取
			topImgSpliceIndex=0,//定义图片从数组中取出的位置
	 ...
	 * @param  index 需要被居中的图片对应的图片信息组的index值
	...
	render() {
		...
		imageDatas.forEach(function(imageData,index){
			...
			imgFigures.push(<ImgFigure key={'imgFigure'+index} data={imageData} ref={'imgFigure'+index}
